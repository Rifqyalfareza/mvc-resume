...
class Customer{
    ...
    public function create($data){
        ...
        $stmt = $this->db->prepare("INSERT INTO customer (nama_customer, singkatan) VALUES (?, ?)");
        $stmt->execute([$customer, $singkatan]);
    }
    public function update($id, $data){
        $customer = strtoupper($data['nama_customer']);
        $singkatan = strtoupper($data['singkatan']);
        $stmt = $this->db->prepare("UPDATE customer SET nama_customer = ?, singkatan = ? WHERE id_customer = ?");
        $stmt->execute([$customer, $singkatan, $id]);
    }
